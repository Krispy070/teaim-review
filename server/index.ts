...
import { contentSecurityPolicy, allowCors, basicSecurityHeaders, requestId } from "./middleware/security";
import { requestLogger } from "./middleware/logger";
import { rateLimit } from "./middleware/rateLimit";
import { badJsonHandler } from "./middleware/bodyErrors";
import health from "./routes/health";
import info from "./routes/info";
import reset from "./routes/admin_reset";
import dev from "./routes/dev_token";
...
import { wireProcessTraps } from "./middleware/procResilience";

// Load environment variables from .env file
import dotenv from 'dotenv';
dotenv.config();

// Stamp process start time for uptime tracking (Fix Pack v233)
process.env.PROC_STARTED_AT = process.env.PROC_STARTED_AT || `${Math.floor(Date.now()/1000)}`;
...
// Hide Express signature (Fix Pack v237)
app.disable("x-powered-by");

// Body limits from central config (Fix Pack v237)
...
// Compression for JSON/text (Fix Pack v237)
app.use(compression());

// Body error handlers (Fix Pack v242)
app.use(badJsonHandler);

// Apply request ID middleware early (Review Patch 1)
app.use(requestId());

// Apply request logger (Fix Pack v229)
app.use(requestLogger({
  sample: 1,                      // 1 = log all; lower to sample
  skipPaths: [/^\/api\/healthz$/, /^\/api\/readyz$/]
}));

// Apply security headers early (Fix Pack v249)
app.use(securityHeaders);
app.use(basicSecurityHeaders());
app.use(contentSecurityPolicy(config.corsAllowed));

// CORS with central config (Fix Pack v237, v249)
app.use(allowCors(config.corsAllowed));
...
(async () => {
  ...
  // Apply rate limiting globally (Fix Pack v231)
  app.use(rateLimit({
    windowMs: 60_000,             // 1 min
    max: 120,                     // 120 writes/min per IP/route
    methods: ["POST","PUT","PATCH","DELETE"]
  }));

  // Health check endpoints (Review Patch 2)
  app.use("/api", health);
  app.get("/health", (_req, res) => res.json({ ok: true }));

  // Info endpoints (Fix Pack v233)
  app.use("/api", info);

  // Dev token endpoint (Fix Pack v248) - only in non-production
  if (process.env.NODE_ENV !== "production") {
    app.use("/api", dev);
  }

  // Admin sandbox reset (Fix Pack v236)
  app.use("/api/admin/sandbox", reset);
  ...
  // Import and mount tickets export routes (Fix Pack v278)
  import("./routes/tickets_export").then(m => app.use("/api/tickets", m.default));

  // Import and mount ingest docs routes (Fix Pack v282)
  import("./routes/ingest_docs").then(m => app.use("/api/ingest", m.default));

  // Import and mount integrations routes (Fix Pack v285)
  import("./routes/integrations").then(m => app.use("/api/integrations", m.default));

  // Import and mount releases summaries cache routes (Fix Pack v287)
  import("./routes/releases_summaries_cache").then(m => app.use("/api/releases", m.default));

  // Import and mount export routes (Fix Pack v293-295)
  import("./routes/timeline_export").then(m => app.use("/api/timeline", m.default));
  import("./routes/risks_export").then(m => app.use("/api/risks", m.default));
  import("./routes/decisions_export").then(m => app.use("/api/decisions", m.default));

  // Import and mount list routes (Fix Pack v297)
  import("./routes/risks_list").then(m => app.use("/api/risks", m.default));
  import("./routes/decisions_list").then(m => app.use("/api/decisions", m.default));

  // Import and mount search routes (Fix Pack v299)
  import("./routes/search_quick").then(m => app.use("/api/search", m.default));
  ...
  // Wire process traps for resilience (Fix Pack v243)
  wireProcessTraps(server);

  // Graceful shutdown (Fix Pack v232)
