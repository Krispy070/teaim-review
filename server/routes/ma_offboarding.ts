...
off.get("/cohorts/:id/offboarding/template.csv", requireProject("member"), async (req,res)=>{
  ...
  res.type("text/csv").send(
    [header.map(escCsv).join(","), ...rows.map((r: any)=>r.map(escCsv).join(","))].join("\r\n")
  );
});

/* POST /api/ma/cohorts/:id/offboarding/import  (form-data: file) body: { projectId } */
off.post("/cohorts/:id/offboarding/import", requireProject("member"), upload.single("file"), async (req,res)=>{
...
off.get("/cohorts/:id/offboarding", requireProject("member"), async (req,res)=>{
  const cid = String(req.params.id||"");
  const result = await db.execute(
...
off.get("/cohorts/:id/offboarding.csv", requireProject("member"), async (req,res)=>{
  ...
  res.type("text/csv").send(
    [head.map(escCsv).join(","), ...rows.map((r:any)=>[
...
off.post("/cohorts/:id/offboarding/bulk", requireProject("member"), async (req,res)=>{
  ...
  const r = await db.execute(
    `update offboarding_rows set ${cols.join(", ")}, updated_at=now()
      where id = any($${params.length-1}::uuid[]) and cohort_id=$${params.length}`,
    params as any
  );
 ...
 */
off.post("/cohorts/:id/offboarding/bulk-by-filter", requireProject("member"), async (req,res)=>{
  const cid = String(req.params.id||"");
 ...
 */
off.post("/offboarding/:rowId/bump", requireProject("member"), async (req,res)=>{
  ...
  const next = bump(base, Number(days)||1);
