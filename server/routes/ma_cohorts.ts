...
ma.get("/cohorts", requireProject("member"), async (req, res) => {
  ...
  // Pagination with clampInt (Fix Pack v239)
...
ma.post("/cohorts/bulk", requireProject("member"), async (req,res)=>{
  ...
  const r = await exec(
    `update cohorts set ${cols.join(", ")}, description=description where id = any($${params.length-1}::uuid[]) and project_id=$${params.length}`,
    params, 12_000, "cohorts:bulk"
  );
...
ma.post("/cohorts/bulk-by-filter", requireProject("member"), async (req,res)=>{
  ...
  const r = await exec(
    ...
    [...setParams, ...whereParams], 12_000, "cohorts:bulk-filter"
  );
  res.json({ ok:true, updated: r.rowCount||0 });
});
