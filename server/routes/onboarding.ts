...
onb.post("/push-to-plan", requireProject("member"), async (req,res)=>{
  ...
  const out = await withTx(async (tx) => {
    const plan = planId ? { id: planId } : (await tx.execute(
  ...
  await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      text: `Onboarding → Plan: pushed ${out.created} task(s) from step ${stepId} into active plan (Plan ${out.planId}).`
    })
  }).catch(()=>{});
  
  res.json({ ok:true, created: out.created, planId: out.planId });
});
