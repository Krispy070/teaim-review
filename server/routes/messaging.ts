...
const ENABLED = (process.env.MESSAGING_ENABLED||"1") !== "0";
const DRY_RUN = (process.env.MESSAGING_DRY_RUN||"0") === "1";
...
msg.post("/post", requireProject("member"), async (req, res) => {
  ...
  const preview = { projectId, category, text, link, enabled: ENABLED, dryRun: DRY_RUN };
  if (!ENABLED || DRY_RUN) {
