...
import { parseISOorNull } from "../lib/parse";
...
relMgr.post("/import", requireProject("member"), requireProjectId(), upload.single("file"), asyncHandler(async (req,res)=>{
  ...
  const ins = await exec(
    `insert into release_imports (project_id, code, year, title, status) values ($1,$2,$3,$4,'imported') returning id`,
    [projectId, code, Number(year), title],
    12_000,
    "release:insert"
  );
  ...
  let total=0;
  const MAX_ROWS = 5000; // safety cap (Fix Pack v238)
  ...
  for (const name of wb.SheetNames){
    ...
    await exec(
      `insert into release_imports (release_id, filename, sheet, rows) values ($1,$2,$3,$4)`,
      [relId, req.file.originalname, name, rows.length],
      12_000,
      "release:import-meta"
    );
  ...
  res.json({ ok:true, releaseId: relId, changes: total, capped: total >= MAX_ROWS });
}));

/* Analyze impact (AI) */
// POST /api/release-manager/:id/analyze { projectId }
relMgr.post("/:id/analyze", ensureUUIDParam("id"), requireProject("member"), asyncHandler(async (req,res)=>{
...
relMgr.post("/:id/testpack", ensureUUIDParam("id"), requireProject("member"), async (req,res)=>{
  const rid = String(req.params.id||""); const { projectId } = req.body||{};
  if (!projectId || !rid) return res.status(400).json({ error:"projectId & releaseId" });
...
relMgr.post("/:id/review", ensureUUIDParam("id"), requireProject("member"), async (req,res)=>{
  const rid = String(req.params.id||""); const { projectId, whenISO=null, link=null } = req.body||{};
  if (!projectId || !rid) return res.status(400).json({ error:"projectId & releaseId" });
  const when = parseISOorNull(whenISO);
  if (whenISO && !when) return res.status(400).json({ error:"invalid date" });
...
4) Decisions / open questions`;
  ...
  await db.execute(
    sql`insert into meetings (project_id, title, starts_at, link, attendees, source, transcript_text, summary)
     values (${projectId},${`Release Review: ${hi.code} ${hi.year}`},${when||null},${link||null},${JSON.stringify([])},'release',null,${brief})`
  );
