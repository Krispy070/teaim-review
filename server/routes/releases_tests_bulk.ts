...
import { ensureUUIDParam } from "../auth/guards";
 ...
 */
rbulk.post("/:id/tests/bulk", ensureUUIDParam("id"), requireProject("member"), asyncHandler(async (req,res)=>{
 ...
 */
rbulk.post("/:id/tests/bulk-by-filter", ensureUUIDParam("id"), requireProject("member"), asyncHandler(async (req,res)=>{
  const rid = String(req.params.id||"");
