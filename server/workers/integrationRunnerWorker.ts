...
async function runSftpPush(projectId:string, integ:any, runId:string){
  ...
  if (source === "latest_artifacts") {
    const sinceHrsStr = String(sinceHrs);
    const rows = (await db.execute(
      sql`select a.storage_path as "p", a.name
         from integration_run_artifacts a
      join integration_runs r on r.id = a.run_id
        where r.integration_id=${integ.id}
          and a.storage_path is not null
          and a.name ~ ${pattern}
          and a.created_at >= now() - (${sinceHrsStr} || ' hours')::interval
  ...
  const note = `SFTP PUSH OK — ${toPush.length} file(s) (parallel=${parallel})\n` + results.join("\n");
  await db.execute(sql`update integration_runs set note=${note} where id=${runId}`);
}
