...
function Group({title, children, collapsed}:{title:string; children:any; collapsed:boolean}){
  return (
    <div className="mb-4">
      {!collapsed && <div className="text-[11px] uppercase tracking-wider text-slate-400 mb-1">{title}</div>}
...
function Item({to, icon:Icon, label, collapsed}:{to:string; icon:any; label:string; collapsed:boolean}){
  ...
  // Active state detection: exact match OR current location is a subpath of the link target
  ...
  const isActive = normalizedLocation === normalizedTo || normalizedLocation.startsWith(normalizedTo + '/');
  return (
    <Link to={to} data-testid={`sidebar-link-${label.toLowerCase().replace(/\s+/g, '-')}`}>
      <div className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer ${isActive?'bg-slate-800 text-sky-300':'text-slate-300 hover:bg-slate-900'} transition-colors`}>
        <Icon size={16} />
...
export default function SidebarV2(){
  ...
  return (
    <div 
      ...
      className="h-full bg-slate-950/90 text-slate-200 p-3 border-r border-slate-800 overflow-y-auto relative"
      style={{ width: collapsed ? 60 : width }}>
      
      {/* Header with collapse button */}
      <div className="flex items-center justify-between mb-2">
        {!collapsed && <div className="text-[11px] uppercase tracking-wider text-slate-400">Menu</div>}
        <button 
