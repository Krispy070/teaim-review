...
export default function WorkstreamArea(){
  ...
  // Extract areaKey from URL path /projects/:projectId/workstreams/:areaKey
  const pathSegments = location.split('/');
  const areaKeyEncoded = pathSegments[pathSegments.length - 1];
  const areaKey = areaKeyEncoded ? decodeURIComponent(areaKeyEncoded) : null;
  ...
  useEffect(() => {
    const currentHash = new URLSearchParams((window.location.hash || "").replace(/^#/, "")).get("tab");
