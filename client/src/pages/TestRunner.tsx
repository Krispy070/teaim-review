...
export default function TestRunner(){
  ...
  async function seed() {
    ...
    try {
      const res = await fetchWithAuth("/admin/test/seed-v2", {
        method: "POST",
  ...
  const tests: T[] = [
    { name: "Areas summary_all", run: async()=> { const r=await fetchWithAuth(`/api/areas/summary_all?project_id=${projectId}`); if(!r.ok) throw new Error("areas summary"); }},
    { name: "Workbooks metrics", run: async()=> { const r=await fetchWithAuth(`/api/workbooks/metrics?project_id=${projectId}`); if(!r.ok) throw new Error("wb metrics"); }},
    { name: "Workbooks export CSV", run: async()=> { const r=await fetchWithAuth(`/api/workbooks/export.csv?project_id=${projectId}`); if(!r.ok) throw new Error("wb export"); }},
    { name: "Digest preview HTML", run: async()=> { const r=await fetchWithAuth(`/api/digest/preview?projectId=${projectId}`); if(!r.ok) throw new Error("digest preview"); }},
    { name: "Docs export ZIP", run: async()=> { const r=await fetchWithAuth(`/api/ingest/export.zip?projectId=${projectId}`); if(!r.ok) throw new Error("docs export"); }},
    { name: "Notifications count", run: async()=> { const r=await fetchWithAuth(`/api/notifications/count?projectId=${projectId}`); if(!r.ok) throw new Error("notify count"); const data=await r.json(); if(typeof data.count !== 'number') throw new Error("invalid response"); }},
    { name: "Releases ICS", run: async()=> { const r=await fetchWithAuth(`/api/releases/ics?projectId=${projectId}`); if(!r.ok) throw new Error("ICS"); }},
