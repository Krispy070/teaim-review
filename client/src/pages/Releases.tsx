...
const SkeletonStyles = () => (
  <style>{`
    ...
    .pulse::after {
      ...
      position: absolute;
      inset: 0;
      background: linear-gradient(90deg, rgba(59,130,246,0.08) 0%, rgba(59,130,246,0.16) 50%, rgba(59,130,246,0.08) 100%);
      animation: pul 1.2s ease-in-out infinite;
      pointer-events: none;
      border-radius: 6px;
    }
    @keyframes pul {
    ...
    .pulse-small{ position:relative; }
    .pulse-small::after{ content:""; position:absolute; inset:-2px;
      border-radius:6px;
      background: radial-gradient(closest-side, rgba(59,130,246,.16), rgba(59,130,246,0));
      animation: puls 1.2s ease-in-out 1; pointer-events:none;}
    @keyframes puls{0%{opacity:.0}30%{opacity:.6}100%{opacity:0}}
  `}</style>
);
...
export default function Releases(){
  ...
  const [bgRefreshing, setBgRefreshing] = useState<boolean>(false);
  const [secondsLeft, setSecondsLeft] = useState<number>(60);
  const [updatedPulse,setUpdatedPulse]=useState<Record<string,boolean>>({});
  const [deltaMap,setDeltaMap]=useState<Record<string,{n:number; mod?:string}>>({});
  let fetching = false;
  ...
  async function prefetchSummaries(force = false) {
    if (fetching) return;
    const now = Date.now();
    const isManual = force;
    if (!isManual && now - lastFetch < 60_000) return;
    fetching = true;
    if (!isManual) setBgRefreshing(true);
    
    try {
      const r = await getJSON(`/api/releases/summaries?projectId=${encodeURIComponent(projectId!)}`);
      if (r.ok) {
        const m: Record<string, any> = { ...sumCache };
        const updatedIds:string[] = [];
        const deltas:Record<string,{n:number; mod?:string}> = {};
        (r.items || []).forEach((it: any) => {
          const prev = m[it.releaseId];
          m[it.releaseId] = it;
          if (!prev || JSON.stringify(prev.gate)!==JSON.stringify(it.gate) ||
              JSON.stringify(prev.modules)!==JSON.stringify(it.modules)) {
            updatedIds.push(it.releaseId);
          }
          if (prev){
            const sum = (mods:any[])=> mods.reduce((a:number,m:any)=> a + (m.total||0), 0);
            const totalPrev = sum(prev.modules||[]);
            const totalNow  = sum(it.modules||[]);
            const diff = Math.max(0, totalNow - totalPrev);

            if (diff>0){
              const firstChanged = (it.modules||[]).find((m:any)=>{
                const pm = (prev.modules||[]).find((x:any)=>x.module===m.module);
                return (m.total||0) > ((pm?.total)||0);
              });
              deltas[it.releaseId] = { n: diff, mod: firstChanged?.module };
            }
          }
        });
        setSumCache(m);
        ...
        if (!isManual){
          setUpdatedPulse(prev=>{
            const next = { ...prev }; updatedIds.forEach(id=> next[id]=true);
            setTimeout(()=> setUpdatedPulse(p=>{
              const cp={...p}; updatedIds.forEach(id=> delete cp[id]); return cp;
            }), 1200);
            return next;
          });

          setDeltaMap(deltas);
          setTimeout(()=> setDeltaMap({}), 1200);
        }
      }
    } catch (err) {
      console.log("Failed to prefetch summaries:", err);
    } finally {
      if (!isManual) setBgRefreshing(false);
      fetching = false;
    }
  }
  ...
  useEffect(() => {
    ...
    prefetchSummaries(true);
    
    const refreshInt = setInterval(() => prefetchSummaries(false), 60_000);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-[11px] opacity-70">
          <span data-testid="text-last-refreshed">
            Last refreshed: {lastFetch ? new Date(lastFetch).toLocaleTimeString() : "—"}
          </span>
          <span data-testid="text-countdown">Auto-refresh in: {secondsLeft}s</span>
          <button 
            className="px-2 py-0.5 border rounded flex items-center gap-1 disabled:opacity-60 hover:bg-slate-800"
            disabled={refreshing}
            onClick={manualRefresh}
            data-testid="button-refresh-summaries"
          >
            {refreshing && <span className="spinner w-3 h-3 border border-slate-500 border-t-transparent rounded-full inline-block"></span>}
...
function ReleaseCard({projectId,rel,crs,onUpdate,sumCache,secondsLeft,bgRefreshing,updatedPulse,deltaMap}:{projectId:string; rel:any; crs:any[]; onUpdate:()=>void; sumCache:Record<string,any>; secondsLeft:number; bgRefreshing:boolean; updatedPulse:Record<string,boolean>; deltaMap:Record<string,{n:number; mod?:string}>}){
    ...
    <div className="border rounded p-2 text-xs bg-white/5" data-testid={`release-card-${rel.id}`}>
      <div className="flex items-center gap-2">
        ...
        <span className="align-middle">
          {!sumCache[rel.id] ? (
            <span className="skeleton-chip" style={{ width: 64, verticalAlign: "middle" }} />
          ) : sumCache[rel.id].gate?.ready ? (
            <span className="text-[11px] px-1 py-0.5 border rounded-full border-emerald-600 text-emerald-300">
              Req {sumCache[rel.id].gate.passed}/{sumCache[rel.id].gate.required}
            </span>
          ) : (
            <span className="text-[11px] px-1 py-0.5 border rounded-full border-amber-600 text-amber-300">
              Req {sumCache[rel.id].gate.passed}/{sumCache[rel.id].gate.required}
            </span>
          )}
        </span>
        <span className="ml-2 align-middle">
          {deltaMap[rel.id] ? (
            <span className="text-[11px] px-1 py-0.5 border rounded-full border-indigo-600 text-indigo-300">
              Δ+{deltaMap[rel.id].n}{deltaMap[rel.id].mod?` (${deltaMap[rel.id].mod})`:""}
            </span>
          ) : null}
        </span>
        <span className="ml-2 text-[11px] opacity-60 align-middle" data-testid={`release-countdown-${rel.id}`}>⟳ {secondsLeft}s</span>
      </div>
      ...
      <div className={`text-[11px] opacity-90 mt-1 ${updatedPulse[rel.id] ? "pulse-small" : ""}`}>
        {!sumCache[rel.id] ? (
          <div className="flex gap-1">
            <span className="skeleton-chip w-24"></span>
            <span className="skeleton-chip w-16"></span>
            <span className="skeleton-chip w-20"></span>
          </div>
        ) : (
          <>
            {sumCache[rel.id].gate?.ready
              ? <span className="px-1.5 py-0.5 border rounded-full border-emerald-600 text-emerald-300 mr-2">
                  Req {sumCache[rel.id].gate.passed}/{sumCache[rel.id].gate.required} ✓
                </span>
              : <span className="px-1.5 py-0.5 border rounded-full border-amber-600 text-amber-300 mr-2">
                  Req {sumCache[rel.id].gate.passed}/{sumCache[rel.id].gate.required}
                </span>}
            {sumCache[rel.id].modules?.slice(0,12).map((m:any)=>(
              <a key={m.module||"Custom"}
                 className="px-1.5 py-0.5 border rounded mr-1 hover:bg-slate-800"
                 title={`✓${m.passed||0} ✗${m.failed||0} ⛔${m.blocked||0} …${m.in_progress||0} • Req ${m.req_passed||0}/${m.req_total||0}`}
                 href={`/projects/${projectId}/releases/${rel.id}/tests?module=${encodeURIComponent(m.module||"Custom")}`}>
                {m.module || "Custom"} ({m.passed||0}/{m.total||0})
              </a>
            ))}
            {sumCache[rel.id].modules?.length>12 && <span className="opacity-60">+{sumCache[rel.id].modules.length-12} more</span>}
          </>
        )}
      </div>
      <div className="mt-1">
...
function CalendarView({ ym, setYm, items, crs, projectId, onUpdate, days, sumCache, secondsLeft, bgRefreshing, updatedPulse, deltaMap }: {
  ...
  onUpdate: () => void;
  days: number[];
  sumCache: Record<string, any>;
  secondsLeft: number;
  bgRefreshing: boolean;
  updatedPulse: Record<string,boolean>;
  deltaMap: Record<string,{n:number; mod?:string}>;
}) {
  return (
    <Card>
      <CardHeader>
            ...
            return (
              <div key={d} className="border rounded p-2 min-h-[120px]" data-testid={`calendar-day-${d}`}>
                ...
                <div className="space-y-1">
                  {inDay.map(r => <ReleaseCard key={r.id} projectId={projectId} rel={r} crs={crs} onUpdate={onUpdate} sumCache={sumCache} secondsLeft={secondsLeft} bgRefreshing={bgRefreshing} updatedPulse={updatedPulse} deltaMap={deltaMap}/>)}
