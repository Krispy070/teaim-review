...
function MyPlanGlanceCard(){
  ...
  useEffect(()=>{ (async()=>{
    const r=await authFetch(`/api/me?projectId=${encodeURIComponent(pid!)}`); 
    if (r.ok) { const j=await r.json(); setMe(j); }
    const pr = await authFetch(`/api/plan/prefs?projectId=${encodeURIComponent(pid!)}`); 
    if (pr.ok){
  ...
  async function loadCounts(){
    ...
    try {
      ...
      console.error("Failed to load plan counts:", err);
    }
  }
    ...
    <div className="p-3 border rounded-2xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {me?.projectRole && (
            <span className="text-[10px] px-1.5 py-0.5 border rounded bg-slate-800 text-slate-300" data-testid="badge-project-role">
              {me.projectRole}
            </span>
          )}
        </div>
...
function OnboardingPushedCard(){
  ...
  useEffect(()=>{ (async()=>{
    const r=await authFetch(`/api/onboarding/pushed_last?projectId=${encodeURIComponent(pid!)}`); 
    if (r.ok) {
      const j=await r.json();
      setLast(j.last||null);
    }
