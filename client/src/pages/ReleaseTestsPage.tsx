...
export default function ReleaseTestsPage() {
  ...
  const [mod, setMod] = useQueryState("module", "");
  const [status, setStatus] = useQueryState("status", "");
  const [order, setOrder] = useQueryState("order", "createdAt");
  const [dir, setDir] = useQueryState("dir", "desc");
  const [reqOnlyRaw, setReqOnlyRaw] = useQueryState("reqOnly", "0");
  const reqOnly = reqOnlyRaw === "1";
  const toggleReqOnly = () => setReqOnlyRaw(reqOnly ? "0" : "1");
