...
export default function PlanBuilderPage(){
  ...
  useEffect(()=>{
    ...
    const qv= params.get("q"); if (qv) setQ(qv);
      ...
      <div className="p-6 space-y-4">
        ...
        {ids.length>0 && (
          <div className="p-2 border rounded-2xl flex items-center gap-2 bg-muted/10" data-testid="bulk-toolbar">
            ...
            <button className="text-xs px-2 py-1 border rounded" onClick={async()=>{
              if (!plan) return;
              for (const id of ids) { await fetchWithAuth(`/api/plan/tasks/upsert`, { method:"POST", body: JSON.stringify({ projectId: pid, planId: plan.id, tasks:[{ id, status:"done" }] }) }); }
              setSel({}); load();
            }} data-testid="button-mark-done">Mark done</button>
            <button className="text-xs px-2 py-1 border rounded" onClick={async()=>{
              if (!plan) return;
              await fetchWithAuth(`/api/plan/tasks/bulk-by-filter`, {
                method:"POST",
